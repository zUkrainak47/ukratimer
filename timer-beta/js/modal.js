import { formatTime, formatSolveTime, formatSolveTimeWithSplits, formatReadableDate, formatDateTime, getEffectiveTime, getSolvePhaseSplits } from './utils.js?v=2026060201';
...
function buildSolveShareText(singleLabel, solveIndex, timeStr, solve) {
    ...
    const lineTimeStr = formatSolveTimeWithSplits(solve);
    return [
        `Generated by UkraTimer on ${formatReadableDate(Date.now())}`,
        `${singleLabel}: ${timeStr}`,
        '',
        `${displayIndex ?? (solveIndex + 1)}. ${lineTimeStr}${getCommentSuffix(solve)}    ${solve.scramble}${getScrambleDateSuffix(solve)}`,
    ].join('\n');
}
...
function rerenderCurrentModalSource() {
    ...
    if (_currentModalSource.type === 'average') {
        showAverageDetail(
            ...
            _currentModalSource.detailOptions || {},
        );
    }
}
...
function rerenderSecondaryModalSource() {
    ...
    if (_secondaryModalSource.type === 'average') {
        showAverageDetail(
            ...
            { ...(_secondaryModalSource.detailOptions || {}), targetLayer: MODAL_LAYER_SECONDARY },
        );
    }
}
