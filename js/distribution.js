import { formatReadableDate, formatTime, getEffectiveTime } from './utils.js?v=2026060801'
...
function buildDistributionTextSummary() {
    ...
    const subject = getDistributionSubjectText()
    if (!distribution.validTimes.length) {
        return [
            `Generated by UkraTimer on ${formatReadableDate(Date.now())}`,
            `${subject} (${formatTimeRange(_state.solveTimestamps)})`,
            '',
...
function getDistributionSubjectText() {
    const sourceLabel = _state?.sourceLabel || 'time'
    if (sourceLabel && sourceLabel !== 'time') {
        return `${sourceLabel} distribution for ${_state.sessionName}`
    }
    return `Time distribution for ${_state?.sessionName || 'Session'}`
}
...
function renderChart(targetCanvas, width, height, { interactive = false, activeIndex = -1 } = {}) {
    ...
    const topTick = Math.ceil(maxCount / yStep) * yStep
    const displayMax = Math.min(
        (maxCount - (topTick - yStep)) <= yStep * 0.1 ? maxCount : topTick,
        Math.max(maxCount, visibleCount),
    )
    ctx.font = tickFont
    ...
    if (interactive) {
        const sourceLabel = _state.sourceLabel && _state.sourceLabel !== 'time' ? `${_state.sourceLabel} ` : ''
        _canvas.setAttribute(
            'aria-label',
            `Distribution of ${distribution.visibleCount} visible ${sourceLabel}solves. Median ${formatTime(distribution.median)}.`
        )
        updateTooltipVisibility()
    }
}
...
function renderDistribution() {
    ...
    _sessionEl.textContent = _state.sourceLabel && _state.sourceLabel !== 'time'
        ? `${_state.sessionName} · ${_state.sourceLabel}`
        : _state.sessionName
...
function setDistributionState(solves, { sessionName = 'Session', times = null, sourceLabel = 'time' } = {}) {
    const sourceTimes = Array.isArray(times) && times.length === solves.length
        ? times
        : solves.map((solve) => getEffectiveTime(solve))
...
export function showTimeDistributionModal(solves, { sessionName = 'Session', times = null, sourceLabel = 'time' } = {}) {
    if (!_overlay) return

    setDistributionState(solves, { sessionName, times, sourceLabel })
...
export function refreshTimeDistributionData(solves, { sessionName = 'Session', times = null, sourceLabel = 'time' } = {}) {
    if (!_overlay?.classList.contains('active')) return
    setDistributionState(solves, { sessionName, times, sourceLabel })
    requestAnimationFrame(() => {
        renderDistribution()
